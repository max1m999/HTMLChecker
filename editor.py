        ...
        for i in self.text():
            if i == "<":
                ...
                if stack and (i ==" " or i == "\r" or i == "\n") :
                    errors.append(f"Пробел после символа <, строка {self.SendScintilla(QsciScintilla.SCI_LINEFROMPOSITION, int(poz))}, индекс: {self.SendScintilla(QsciScintilla.SCI_GETCOLUMN, int(poz))}")
                stack.pop() 
            index += 1    
        return errors
